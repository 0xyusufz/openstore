...
export async function runStorageNodeCli(argv: string[], options: StorageNodeCliOptions = {}): Promise<number> {
  ...
  try {
    ...
    if (parsed["allocation-path"]) config.allocationPath = parsed["allocation-path"];
